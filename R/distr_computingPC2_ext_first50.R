...
treesblck <- rodtrees
#treesblck[-7][1:35] si sirve
# special case
#extPC2shifts_7 <- est_shifts_phyl(treesblck[[7]])
extPC2shifts_f34 <- future_map(treesblck[-7][1:35],est_shifts_phyl)
extPC2shifts_f34[[35]] <- NULL
# extPC2shifts_36 <- est_shifts_phyl(treesblck[[36]])
# extPC2shifts_37 <- est_shifts_phyl(treesblck[[37]])
# extPC2shifts_38 <- est_shifts_phyl(treesblck[[38]])
#rm(extPC2shifts_7,extPC2shifts_36,extPC2shifts_37,extPC2shifts_38)
#rm(extPC2shifts_36to38)
extPC2shifts_7_36to38 <- list(extPC2shifts_7,extPC2shifts_36,extPC2shifts_37,extPC2shifts_38)
extPC2shifts_f39to50 <- future_map(treesblck[39:50],est_shifts_phyl)
extPC2shifts_f50to70 <- future_map(treesblck[51:70],est_shifts_phyl)
extPC2shifts_f70to100 <- future_map(treesblck[71:100],est_shifts_phyl)

extPC2shifts_all100 <- 
  flatten(mget(ls(pattern = "^extPC2s")))
extPC2shifts_all100
rm(extPC2shifts_all100)

saveRDS(object = extPC2shifts_all100,file = "2021_conv/out/emodPC2_ext_all100_bic_pca.rds")
extPC2shifts_all100 <- readRDS(file="2021_conv/out/emodPC2_ext_all100_bic_pca.rds")
extPC2shifts_all100[[1]]$nShifts

nshiftvec <- map(extPC2shifts_all100,'nShifts') %>% simplify()
names(nshiftvec) <- c(1:100)
nshiftvec
noshifttrees <- which(nshiftvec==0)

extPC2shifts_all100[-noshifttrees]

PC2ext_conv_regs_all100 <- 
  future_map(extPC2shifts_all100[-noshifttrees],estimate_convergent_regimes,nCores=4)

PC2ext_conv_regs_all100
saveRDS(object = PC2ext_conv_regs_all100, file = "2021_conv/out/PC2_conv_regs_ext-all100trees.rds")

AFDPC2ext_conv_regs_first25
saveRDS(object = PC2ext_conv_regs_first50,file = "2021_conv/out/PC2_conv_regs_ext-first50trees.rds")
