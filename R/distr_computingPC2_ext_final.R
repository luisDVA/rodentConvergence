...
est_shifts_phyl <- function(treeph) {
  ...
  eModel <- estimate_shift_configuration(adjdata$tree, adjdata$Y, max.nShifts = 21,
                                         criterion = "BIC",nCores = 4)
  eModel
}


extPC2shiftsrem1 <- future_map(rodtrees[1:50],est_shifts_phyl)
extPC2shiftsrem2 <- future_map(rodtrees[51:100],est_shifts_phyl)
extPC2shiftsrem1
saveRDS(object = extPC2shiftsrem1,file = "2021_conv/out/PC2_ext-first50trees_new.rds")
saveRDS(object = extPC2shiftsrem2,file = "2021_conv/out/PC2_ext-second50trees_new.rds")

extPC2shiftsrem1 <- readRDS("2021_conv/out/PC2_ext-first50trees_new.rds")
extPC2shiftsrem2 <- readRDS("2021_conv/out/PC2_ext-second50trees_new.rds")
extPC2shift_all100 <-  c(extPC2shiftsrem1,extPC2shiftsrem2)

nshiftvec <- map(extPC2shift_all100,'nShifts') %>% simplify()
nshiftvec
names(nshiftvec) <- c(1:100)
nshiftvec
noshifttrees <- which(nshiftvec==0)
noshifttrees
forModnames <- names(nshiftvec[-noshifttrees])
forModnames

PC2ext_conv_regs_all89 <- 
  map(extPC2shift_all100[-noshifttrees],estimate_convergent_regimes,nCores=4)

names(PC2ext_conv_regs_all89) <- glue("2021_conv/out/emodPC2_ext{tree}_bic_pca.rds",tree=forModnames)
names(PC2ext_conv_regs_all89)
saveRDS(object = PC2ext_conv_regs_all89,file = "2021_conv/out/processed_l1ouModels/PC2_convRegs_extAll.rds")
